...
class MailUserRobotAI(MailUserRobot):
    ...
    def __msg_get_data(msg):
        ...
        if isinstance(subject,bytes):
            try:
                ...
                subject=subject.decode('gbk',errors='replace')
        ...
        try:
            ...
            payload=payload.decode('gbk',errors='ignore')
    ...
    async def __taskMain(self,userFrom,msg):
        ...
        subject_data=quopri.encodestring(
                subject.encode('gb2312',errors='replace'))
        subject_data=f'=?gb2312?Q?{subject_data.decode('ascii',errors='ignore')}?='
        msg_reply['Subject']=subject_data
        content=content.encode(reply_charset,errors='replace')
        msg_reply.set_payload(content.decode(reply_charset),
